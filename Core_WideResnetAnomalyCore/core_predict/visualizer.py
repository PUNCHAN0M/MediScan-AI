...
"""
Visualization utilities for WideResNet pill inspection.

Single Responsibility:
- Draw bboxes, labels, overlays
- Draw anomaly heatmaps
- No business logic
"""
import cv2
import numpy as np
...
COLOR_BLACK = (0, 0, 0)
COLOR_YELLOW = (0, 255, 255)    # Yellow for warnings
...
def draw_heatmap_overlay(
    ...
    Args:
        image: BGR image
        heatmap: 2D float array (anomaly scores)
        alpha: Blending alpha (0=image only, 1=heatmap only)
    ...
    hmin, hmax = heatmap_norm.min(), heatmap_norm.max()
    if hmax > hmin:
        heatmap_norm = (heatmap_norm - hmin) / (hmax - hmin)
