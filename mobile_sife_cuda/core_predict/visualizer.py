...
"""
Visualization utilities for pill inspection.

Responsibilities:
- Draw bounding boxes, labels, overlays
- Summary display with anomaly counts
...
def put_text_top_right(
    ...
    """Draw text at top-right corner with background."""
    font = cv2.FONT_HERSHEY_SIMPLEX
    (tw, th), _ = cv2.getTextSize(text, font, font_scale, thickness)
    h, w = img.shape[:2]
    x = w - tw - margin
...
def put_text_top_left(
    ...
    """Draw text at top-left corner with background."""
    font = cv2.FONT_HERSHEY_SIMPLEX
    (tw, th), _ = cv2.getTextSize(text, font, font_scale, thickness)
    x = margin
    y = th + margin
...
def draw_pill_results(
    ...
    """Draw detection results with bboxes and labels."""
    vis = frame.copy()
    for r in results:
