# ResnetPatchCore/core_predict/inspector.py
"""
PillInspectorResNet - Main inspection class with vote accumulation.

Uses ResNet18 backbone + Color Features for better color anomaly detection.

Single Responsibility:
- Detect pills using YOLO
- Classify anomalies using ResNet PatchCore
...
from ResnetPatchCore.core_predict.visualizer import draw_pill_results, draw_summary
...
class InspectorConfig:
    """Configuration for PillInspectorResNet."""
    ...
    # PatchCore (ResNet specific)
    model_size: int = 256
    grid_size: int = 28       # Smaller patches for better small defect detection
    k_nearest: int = 19
    use_color_features: bool = True   # Enable color features
    use_hsv: bool = True              # Enable HSV color space
    color_weight: float = 1.0         # Weight for color features
...
# INSPECTOR
# =========================================================
class PillInspectorResNet:
    """
    Pill anomaly inspector using ResNet18 + Color Features.
    
    This version is specifically designed for cases where color matters:
    - Differentiating white vs black pills
    - Detecting small colored defects
    - Color contamination detection
    
    Usage:
        inspector = PillInspectorResNet()
        for frame in frames:
    ...
    def _init_models(self) -> None:
        ...
        # ResNet PatchCore with Color Features
        self._patchcore = ResNetPatchCore(
        ...
        # YOLO detector (shared from MobilenetPatchCore)
